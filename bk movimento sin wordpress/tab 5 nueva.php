...
function fnDiario_Detalle_Validar($decCantidad,$decAlimento,$decTiempo){
    ...
    if ( empty( $decCantidad ) ) {
        $reg_errors->add('field', 'Ingresar Porcion o Equivalencia');
    }
    if ( empty( $decTiempo ) ) {
        ...
        echo '
            ...
            <form action="'.strtok($_SERVER["REQUEST_URI"],'?').'?action=tab_Paso_5&tab_Diario_'.$diario->intId.'" method="post" class="row" style="margin-bottom: 0px;">
                ...
                <div style="display: inline-block;width: 100% !important;">
                    <div class="col-md-2 col-xs-6 col-sm-6" style="display: grid;padding-right: 0px;">
                    ...
                    <div class="col-md-2 col-xs-6 col-sm-6" style="display: grid;">
                        <div style="width: 100% !important;">
                            <label for="intDiarioDet_Cantidad">Por Porcion <strong>*</strong></label>
                            <label for="intDiarioDet_Cantidad" style="float: right;"></label>
                        </div>
                        <input type="number" name="intDiarioDet_Cantidad" id="intDiarioDet_Cantidad_'.$diario->intId.'" min="0" max="99" step="0.01">
                    </div>
                    <div class="col-md-2 col-xs-6 col-sm-6" style="display: grid;padding-left: 0px">
                        <label for="intDiarioDet_Cantidad_UM"><span id="span_um_porcion">o Por Equivalencia</span><strong>*</strong></label>
                        <input type="number" name="intDiarioDet_Cantidad_UM" id="intDiarioDet_Cantidad_UM_'.$diario->intId.'" min="0" max="99" step="0.01">
                    </div>
                    <script>
                        $("#intDiarioDet_Cantidad_'.$diario->intId.'").keyup(function() {
                            fnVerificarEquivalencia(1, '.$diario->intId.');
                        });
                        $("#intDiarioDet_Cantidad_UM_'.$diario->intId.'").keyup(function() {
                            fnVerificarEquivalencia(2, '.$diario->intId.');
                        });
                    </script>
                    <div class="col-md-4 col-xs-6 col-sm-6" style="display: grid;padding-left: 0px;padding-right: 0px;">
                        <label for="intDiarioDet_Alimento">Alimento <strong>*</strong></label>
                        <select name="intDiarioDet_Alimento" id="intDiarioDet_Alimento_'.$diario->intId.'">';
                        foreach ($listAlimentos as $alimento) {
                            ...
                            echo '<option value="'.$alimento->intId.'">'.($alimento->decPorcion + 0).' '.$alimento->strUnidadMedida.' '.$alimento->strAlimento.' contiene ('.$strPCGVL.')</option>'; }
                    echo '</select>
                    </div>
                    <div class="col-md-2 col-xs-6 col-sm-6" style="display: grid;padding-left: 0px;">
...
<script>
    function fnVerificarEquivalencia(intOpcion, decDiario) {
        var decPorcion = (intOpcion == 1 ? $('#intDiarioDet_Cantidad_' + decDiario).val() : 0);
        var decEquivalencia = (intOpcion == 2 ? $('#intDiarioDet_Cantidad_UM_' + decDiario).val() : 0);
        var strValor = $('#intDiarioDet_Alimento_' + decDiario).val();
        console.log('decPorcion',decPorcion);
        console.log('decEquivalencia',decEquivalencia);
